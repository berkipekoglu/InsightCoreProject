...
// Modal Component
const ScriptModal = ({ project, onClose }: { project: Project | null, onClose: () => void }) => {
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50">
            <div className="bg-gray-800 p-8 rounded-lg max-w-2xl w-full">
                <h2 className="text-xl font-bold mb-4">Installation Script for {project.name}</h2>
                <p className="text-gray-400 mb-4">Copy and paste this script into the `&lt;head&gt;` tag of your website.</p>
                <pre className="bg-gray-900 p-4 rounded overflow-x-auto">
                    <code>{script}</code>
                </pre>
                <div className="flex justify-end gap-4 mt-6">
                    <button onClick={() => navigator.clipboard.writeText(script)} className="bg-blue-600 hover:bg-blue-700">
                        Copy to Clipboard
                    </button>
...
export default function DashboardPage() {
        ...
        return (
            <main className="flex min-h-screen flex-col items-center justify-center p-24">
                <div className="w-full max-w-2xl text-center">
                    ...
                    <form onSubmit={handleCreateProject} className="flex flex-col items-center gap-4">
                        <input
                            type="text"
                            ...
                            required
                        />
                        <button type="submit" className="w-full max-w-md">Create Project</button>
                    </form>
                </div>
            </main>
        );
    ...
    return (
        <>
            <ScriptModal project={selectedProject} onClose={() => setSelectedProject(null)} />
